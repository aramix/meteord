...
docker run -d \
    ...
    -e BUNDLE_URL=https://aramix-meteord-tests.s3-us-west-2.amazonaws.com/meteord-test-bundle.tar.gz \
    -e REBUILD_NPM_MODULES=1 \
    -p 9090:80 \
    "aramix/meteord:node-${NODE_VERSION}-base"
...
if [[ $appContent != *"aramix/meteord:bin-build"* ]]; then
